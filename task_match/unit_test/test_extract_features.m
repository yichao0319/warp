...
function test_extract_features()
    ...
    cnt = 1;
    test{cnt} = 'shift, coeff, num=0 (best)';
    sync{cnt} = 'shift';
    ...
    for ti = 1:length(test)
        ...
        opt = ['sync=''' char(sync{ti}) ''',metric=''' char(metric{ti}) ''',num=' num2str(num(ti))];
        X_feature = extract_features(X{ti}, gt_class{ti}, opt);
        for ti = 1:length(X_feature)
            fprintf('row %d: #feature = %d\n', ti, size(X_feature{ti},1));
        end
        input('========================================\n');
    end
end