...
double dtw_c_orig(double *s, double *t, int w, int ns, int nt, int k)
{
    double d=0;
        ...
        int nrhs, const mxArray *prhs[])
{
    ...
    if(nrhs!=2&&nrhs!=3)
    {
        mexErrMsgIdAndTxt( "MATLAB:dtw_c_orig:invalidNumInputs",
                "Two or three inputs required.");
    }
    if(nlhs>1)
    {
        mexErrMsgIdAndTxt( "MATLAB:dtw_c_orig:invalidNumOutputs",
                "dtw_c_orig: One output required.");
    }
    ...
    else if(nrhs==3)
    {
        ...
        {
            mexErrMsgIdAndTxt( "MATLAB:dtw_c_orig:wNotScalar",
                    "dtw_c_orig: Input w must be a scalar.");
        }
    ...
    if(mxGetN(prhs[1])!=k)
    {
        mexErrMsgIdAndTxt( "MATLAB:dtw_c_orig:dimNotMatch",
                    "dtw_c_orig: Dimensions of input s and t must match.");
    }  
